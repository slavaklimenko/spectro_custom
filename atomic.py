...
class line():
    ...
    """
    def __init__(self, name, l, f, g, logN=None, b=None, z=0, nu_u=None, j_u=None, nu_l=None, j_l=None, descr='', ref='',Aki=None,brgt=None):
        self.name = name
...
class atomicData(OrderedDict):
    ...
    def makedatabase(self):
        ...
        self.read_NiII()
        self.read_IR_HI_HeII_lines()
        self.read_IR_Mol_lines()
        self.read_IR_Metal_lines()
        self.writedatabase()
    ...
    def read_IR_HI_HeII_lines(self):
        with open(self.folder + r'/data/IR_lines/HI-DI-HeII-lines.dat', newline='') as f:
            data = f.readlines()
        for d in data:
            if d[0] != '#':
                name = d[0:6]
                ...
                if name not in self:
                    self[name] = e(name)
                lam = float(d[17:31]) * 1e4
                self[name].lines.append(line(name, lam, 0e0, 0e0, descr=' '.join(d[14:25])))

    def read_IR_Mol_lines(self):
        with open(self.folder + r'/data/IR_lines/IR_molecular_lines.dat', newline='') as f:
            data = f.readlines()
        for d in data:
            if d[0] != '#':
                words = d.split()
                if words[0] not in self:
                    self[words[0]] = e(words[0])
                lam = float(d[30:42])*1e4
                self[words[0]].lines.append(line(words[0], lam, 0e0, 0e0, descr=' '.join(d[14:25])))

    def read_IR_Metal_lines(self):
        with open(self.folder + r'/data/IR_lines/Fine-structure.lines', newline='') as f:
            data = f.readlines()
        for d in data:
            if d[0] != '#':
                words = d.split()
    ...
    def LLS_COS_list(self, lines=True):
        linelist = ['HI 1215.67',
                    'HI 1025',
                    'NI 1134.16',
                    'NI 1134.41',
    ...
    def IR_Metal_list(self, lines=True):
        data = np.genfromtxt(self.folder + r'/data/IR_lines/Fine-structure.lines', skip_header=0, usecols=(0), dtype=(str))
        return self.list(np.unique(data))

    def IR_Molec_list(self, lines=True):
        data = np.genfromtxt(self.folder + r'/data/IR_lines/IR_molecular_lines.dat', skip_header=0, usecols=(0), dtype=(str),comments='#')
        return self.list(np.unique(data))

    def IR_atomic_list(self, lines=True):
        with open(self.folder + r'/data/IR_lines/HI-DI-HeII-lines.dat', newline='') as f:
            data = f.readlines()
        name = []
        for d in data:
            if d[0] != '#':
                name.append(d[0:6].replace(" ", ""))
        data = np.array(name)
        print('data',data)
        return self.list(np.unique(data))
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        A.IR_Molec_list()
        d = A.IR_HI_HeII_list()
        print([str(line) for line in A.list('CO')])
