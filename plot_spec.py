...
class plot_spec(list):
    ...
    def specify_rects(self, rect_pars):
        ...
        top = 1 - delta
        for r in rect_pars:
            panel_h = (r.height - r.h_indent - r.row_offset * (r.n_rows - 1)-delta) / r.n_rows
            panel_w = (r.width - r.v_indent - r.col_offset * (r.n_cols - 1)-delta) / r.n_cols
...
class plotline():
    ...
    def plot_line(self):
        ...
        if self.gray_out:
            ...
            self.ax.errorbar(self.spec.x, self.spec.y, lw=self.parent.lw_spec, elinewidth=elinewidth,
                             drawstyle='steps-mid', color='0.5', ecolor=ecolor, capsize=capsize, zorder=0)
            self.ax.errorbar(self.spec.x, self.spec.y, self.spec.err, fmt='none', elinewidth=elinewidth, color='0.5', ecolor=ecolor, capsize=capsize, zorder=0)
            k = (self.points == 0)
        ...
        self.ax.errorbar(self.spec.x, self.spec.y, lw=self.parent.lw_spec, elinewidth=elinewidth,
                         drawstyle='steps-mid', color='k', ecolor=ecolor, capsize=capsize, zorder=1)
        #self.ax.errorbar(self.spec.x, self.spec.y, self.spec.err, fmt='none', elinewidth=elinewidth, color='k',
        #                 ecolor=ecolor, capsize=capsize, zorder=1)
        ...
        self.ax.tick_params(which='both', width=1, direction='in')
        self.ax.tick_params(which='major', length=5, direction='in')
        self.ax.tick_params(which='minor', length=3, direction='in')
        self.ax.tick_params(axis='both', which='major', labelsize=self.font-2, direction='in')
        ...
        if self.show_comps and self.show_fit:
            for k in range(self.num_comp):
                ...
                if 0 and 'HI' in self.name:
                    self.ax.plot([v-81.6, v-81.6], [self.y_min, self.y_max], color=self.parent.color[k], linestyle=':', lw=self.parent.lw[k])
                #if 1 and 'SII' in self.name or 'SiII' in self.name:
                #if 0 and 'HI' in self.name or 'SII' in self.name or 'SiII' in self.name or 'SiIII' in self.name or 'CII' in self.name or 'NV' in self.name:
                #if 1 and 'HI' in self.name or 'SII' in self.name  or 'SiIII' in self.name: # or 'NV' in self.name:
                if 1 and 'SiIII' in self.name:
                #if 0:
                    lines = {}
                    #lines['CII'] =  1334.53
                    #lines['CII$^*$'] = 1335.70
                    lines['SII 1250'] = 1250.57
                    lines['SII 1253'] = 1253.80
                    lines['SII 1259'] = 1259.51
                    #lines['SiII 1260'] = 1260.42
                    lines['SiII 1304'] = 1304.37
                    lines['SiII 1260'] = 1260.42
                    lines['HI 917'] = 917.18
                    lines['HI 918'] = 918.12
                    lines['HI 923'] = 923.15
                    ...
                    lines['HI 930'] = 930.74
                    lines['HI 949'] = 949.74
                    lines['HI 1025'] = 1025.72
                    lines['NV 1242'] = 1242.80
                    lines['NV 1238'] = 1238.82
                    lines['CI 1277'] = 1277.24
                    lines['CI 1280'] = 1280.13

                    wref = self.wavelength
                    for l in lines.keys():
                        wl = lines[l]
                        v1 = (wl*(1+self.parent.comps[k])/wref/ (1 + self.parent.z_ref) - 1) * 299794.26
                        if v1 < self.x_max and v1 > self.x_min:
                            self.ax.plot([v1, v1], [1.1, 1.3], color=self.parent.color[k], linestyle='-',
                                         lw=1.0)
                            self.ax.text(v1-70,1.1,l,rotation=90,color=self.parent.color[k], fontsize=self.font_labels-2)
