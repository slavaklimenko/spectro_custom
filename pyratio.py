...
class par():
    ...
    def __init__(self, name, prior=None, parent=None):
        ...
        if name == 'nH':
            self.mol_fr = 0 # in log10 s !!!
            self.label = r'$\log\,$nH' # total hydrogen density
            self.init = 2
            self.range = [0, 5]
            self.init_range = 0.3
       ...
       # >>> electron number density
        ...
        if name == 'trap':  # coefficient of photon trapping for CO lines
            self.label = r'$\log\beta$ Trapping'
            self.init = [1,1,1,1]
            self.range = [0, 1]
            self.init_range = 1
...
class pyratio():
    ...
    def load_sed(self):
        ...
        if 'Draine' in self.sed_type:
            ...
            #print('test draine:',self.draine(10))
    ...
    def u_CMB(self, nu):
        ...
        print('temp_cmb',temp,self.Planck2)
        R = np.zeros_like(nu)
        ind = np.where(nu != 0)
    ...
    def balance(self, name=None, debug=None):
        ...
        tmp = np.zeros([speci.num, speci.num])
        #self.timer.time('rates in')

        if 'trap' in self.pars:
            beta = 1
            if 1:
                for u in range(speci.num):
                    for l in range(speci.num):
                        if speci.Aij[u,l]>0:
                            #k  = 1.0 #(speci.Aij[u,l]/speci.Aij[1,0])*(speci.Eij[1,0]/speci.Eij[u,l])**3.*(speci.g[u]/speci.g[l]/3.)
                            #texc = 3.0 #
                            #if u<3:
                            #    m = 0 # u-1
                            #elif u<4:
                            ...
                            #else:
                            #    m = u-2
                            #k *= speci.g[m] * np.exp(-speci.Eij[m, 0] / texc)
                            #tmp[u, l] =1/(1+0.5*self.pars['b_trap'].value*k)
                            if u<5:
                                tmp[u,l] = self.pars['trap'].value[l]
                            else:
                                tmp[u, l] = 1
                            tmp[l,u] = tmp[u,l]
                #for u in range(speci.num):
                #    for l in range(speci.num):
                        #if speci.Aij[u, l] > 0:
                        #    speci.Aij[u, l] *= tmp[u,l]
                        #if speci.Bij[u, l] > 0:
                        #    speci.Bij[u, l] = speci.Bij[u, l]*beta*tmp[u,l]
        else:
            beta = 1
            for u in range(speci.num):
                for l in range(speci.num):
                    ...
                    #    speci.Aij[u, l] *=1
                    #if speci.Bij[u, l] > 0:
                    #    speci.Bij[u, l] = speci.Bij[u, l]*beta
        ...
        if debug in [None, 'C']:
            for u in range(speci.num):
                for l in range(speci.num):
                    if any(x in self.pars.keys() for x in ['n', 'e', 'H2', 'H','nH']):
        ...
        if 'rad' in self.pars:
                   ...
                   for u in range(speci.num):
                       for l in range(speci.num):
                           if 'UV' in self.pars:
                               ...
                               print(self.pumping_rate(speci, u, l))
           ...
           if debug in [None, 'IR']:
               if self.radiation == 'full':
                   for u in range(speci.num):
                       for l in range(speci.num):
                           W[u, l] += speci.Bij[u, l]*tmp[u,l] *self.rad_field(speci.Eij[u, l])*beta
               if self.radiation == 'simple':
    ...
    def collision_rate(self, speci, u, l, verbose=False):
        ...
        for p in self.pars:
            ...
            if p in 'n':
                m_fr = 10 ** self.pars['f'].value if 'f' in self.pars else self.mol_fr
            ...
            if p in 'nH':
                m_fr = 10 ** self.pars['f'].value if 'f' in self.pars else self.mol_fr
                f_HI, f_H2 = (1-m_fr) , m_fr / 2
                #print('h',10 ** self.pars['nH'].value* f_HI, 'h2',10 ** self.pars['nH'].value *f_H2 )
                coll += 10 ** self.pars['nH'].value * speci.coll['H'].rate(u, l, self.pars['T'].value) * f_HI
                otop = 9 * np.exp(-170.6 / 10 ** (self.pars['T'].value))
                coll += 10 ** self.pars['nH'].value * speci.coll['pH2'].rate(u, l, self.pars['T'].value) * f_H2 / (1 + otop)
                coll += 10 ** self.pars['nH'].value * speci.coll['oH2'].rate(u, l, self.pars['T'].value) * f_H2 * otop / (1 + otop)
                if self.f_He != 0:
                    coll += 10 ** self.pars['nH'].value * speci.coll['He4'].rate(u, l, self.pars['T'].value) * self.f_He
    ...
    def rad_field(self, e, sed_type=None):
        ...
        if sed_type is None and self.CMB or 'CMB' == s:
            ...
            #temp = self.pars['CMB'].value if 'CMB' in self.pars.keys() else 2.732 * (1 + self.z)/1.438
        ...
        if s == 'Draine':
            ...
            #if self.draine(e[m]) * 10 ** self.pars['rad'].value>0:
            #    print('draine contr',e[m])
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        pr.pars['nH'].value = 1.5
        pr.pars['T'].value = np.log10(100.)
        pr.pars['f'].value = -0.3
        pr1 = pr.predict(level=-1)
        pr = pyratio(z=2.6954)
        pr.add_spec('CO', num=10)
        pr.set_pars(['rad', 'nH', 'T', 'f'])
        pr.pars['nH'].value = -5
        pr.pars['T'].value = np.log10(100.)
        pr.pars['f'].value = -0.3
        pr2 = pr.predict(level=-1)
        print(pr1)
        print(pr2)
        print([pr1[k]-pr2[k] for k in range(len(pr1))])
        print([14.21+pr1[k]  for k in range(len(pr1))])
        if 0:
            W = pr.balance(debug='IR')
            A = pr.balance(debug='A')
            C = pr.balance(debug='C')
            B = W + A
            T = B + C

    if 0:
        pr = pyratio(z=1.7)
        lam = np.linspace(0,30,100)
        flux = pr.u_CMB(lam)
        pr = pyratio(z=1.2)
        lam = np.linspace(0,30,100)
        flux2 = pr.u_CMB(lam)
        plt.plot(lam,flux,color='red')
        plt.plot(lam, flux/2, '--',color='red')
        plt.plot(lam, flux2, color='green')
        plt.show()
        lam_x = np.linspace(1e6,2.2e9,1e4)
        koef = 1e32*4 * np.pi * 1.05e-27 * 3e10 ** 2 / np.array(lam_x) ** 5
        lam_x = lam
        spec_cmb = (1 / (np.exp(1.438 * lam_x / 2.732) - 1))
        spec_cmb2 = (1 / (np.exp(1.438 * lam_x / 2.732/2.7) - 1))

        #field[m] += self.Planck1 * e[m] ** 3 / (np.exp(self.Planck2 / temp * e[m]) - 1)

        y = spec_cmb2/spec_cmb
        plt.plot(lam_x, spec_cmb2/spec_cmb)
        plt.show()
    ...
    if 1:
        ...
        pr.add_spec('CO', num=10)
        pr.set_pars(['rad','nH','T','f'])
        pr.pars['nH'].value = 2.48
        pr.pars['T'].value = np.log10(107)
        pr.pars['f'].value = -0.3
        results = pr.predict(level=0)
        print('res:',results)
        g = pr.species['CO'].g
        E = pr.species['CO'].E
        f = [-E[i]*1.428/np.log(10**results[i]/g[i]) for i in [1,2,3]]
        print('f',f)
        print('tcmb:',2.725*3.4)
