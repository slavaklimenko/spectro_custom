...
function fitMCMC(spec, ppar, add; sampler="Affine", tieds=Dict(), prior=nothing, nwalkers=100, nsteps=1000, nthreads=1, thinning=1, init=nothing, opts=0)
	...
	function lnlike(p)
		...
		# add constraints to the ionization structure

		# constraints for H2 on increasing b parameter with J level increase
		#println("ionization_fr")
		if opts["ionization_fr"] == false
			for (k, v) in pars
				if occursin("SiII", k) & occursin("N_", k)
					for (k1, v1) in pars
                        if occursin(k[1:4], k1) & occursin("HI", k1)
                            NHI = v1.val
                            NSi = 0
                            for (k2, v2) in pars
                                if occursin(k[1:4], k2) & occursin("SiII", k2)
                                    NSi += 10^v2.val
                                    #println("k1,k2",k1,k2)
                                end
                            end
                            if NSi>0
                                NSi = log10(NSi)
                                Si2HI = NSi - NHI
                                #println("retval_before",retval)
                                #println("comp ", k[3:3], " Si2HI ",Si2HI, ": NSi: ", NSi, " NHI: ",NHI)
                                if Si2HI< -4.5
                                    retval -=1e10
                                end
                                if Si2HI> -1
