                    ...
                    if (ind == self.parent.s.ind) or ('all' in args):
                        ...
                        if 'err' in args:
                            if len(s.spec.raw.err) == 0:
                                self.parent.s[self.parent.s.ind].spec.raw.err = np.ones_like(s.spec.raw.x) * float(args[argind])
                            self.parent.s[self.parent.s.ind].spec.raw.err[mask] = float(args[argind]) * np.ones_like(s.spec.raw.x[mask])

                        elif 'cont' in args:
                            self.parent.s[self.parent.s.ind].set_spline([s.spec.raw.x[mask][0], s.spec.raw.x[mask][-1]], [float(args[argind]), float(args[argind])])
                            self.parent.s.redraw()

                        else:
                            self.parent.s[self.parent.s.ind].spec.raw.y[mask] = float(args[argind]) * np.ones_like(s.spec.raw.x[mask])
                            if len(s.spec.raw.err) == 0:
                                self.parent.s[self.parent.s.ind].spec.raw.err = np.ones_like(s.spec.raw.x) * float(args[argind]) * np.ones_like(s.spec.raw.x[mask])
                            self.parent.s[self.parent.s.ind].spec.raw.err[mask] = float(args[argind]) * np.ones_like(s.spec.raw.x[mask])
