...
def spectres(spec_wavs, spec_fluxes, resampling, spec_errs=None, spec_errs_lower=None):
    ...
    if spec_errs_lower is not None:
        if spec_errs_lower.shape != spec_fluxes.shape:
            sys.exit("If specified, spec_errs must be the same shape as spec_fluxes.")
        else:
            resampled_errs_lower = np.copy(resampled)
    ...
    for j in range(len(filter_lhs) - 1):
        ...
        if spec_fluxes.ndim == 1:
            ...
            if stop == start:
                ...
                if spec_errs_lower is not None:
                    resampled_errs_lower[j] = spec_errs_lower[start]
            ...
            # Otherwise multiply the first and last old bin widths by P_ij, all the ones in between have P_ij = 1
                ...
                if spec_errs is not None:
                    resampled_errs[j] = np.sqrt(
                        ...
                        spec_widths[start:stop + 1])
                if spec_errs_lower is not None:
                    resampled_errs_lower[j] = np.sqrt(
                        np.sum((spec_widths[start:stop + 1] * spec_errs_lower[start:stop + 1]) ** 2)) / np.sum(
                        spec_widths[start:stop + 1])
        ...
        # The same as above, except operates on each row of the array, resampling all of the input models
            ...
            if stop == start:
                ...
                if spec_errs_lower is not None:
                    resampled_errs_lower[j, :] = np.sqrt(
                        np.sum((np.expand_dims(spec_widths[start:stop + 1], axis=1) * spec_errs_lower[start:stop + 1]) ** 2,
                               axis=0)) / np.sum(spec_widths[start:stop + 1])
    ...
    if spec_errs is not None and spec_errs_lower is None:
        return resampled, resampled_errs
    if spec_errs_lower is not None:
        return resampled, resampled_errs,resampled_errs_lower
