...
class plotSpectrum(pg.PlotWidget):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.w_status or self.e_status:
            ...
            #mask*= s.spec.y()[mask] / cont(s.spec.x()[mask]) > 2*s.spec.err()[mask]
            if np.sum(mask) > 0:
                ...
                if len(s.cont.x) > 0 and s.cont.x[0] < x[0] and s.cont.x[-1] > x[-1]:
                    ...
                    if hasattr(self.parent.abs, 'reference'):
                        if self.w_status:
                            ...
                            showproc = 1
                            if showproc:
                                fig, bx = plt.subplots(4,1,sharex=True)
                                func_sn = (cont(s.spec.x()) - s.spec.y())/s.spec.err()
                                bx[0].step(s.spec.x()[mask],func_sn[mask],where='mid',label='C-S/N')
                                mask1 = mask*cont(s.spec.x()) - s.spec.y() > 1*s.spec.err()
                                #bx[0].step(s.spec.x()[mask1], func_sn[mask1], where='mid',color='red')
                                bx[0].plot(s.spec.x()[mask1], func_sn[mask1], marker='x', color='red')

                                func_ew = (cont(s.spec.x()[mask]) - s.spec.y()[mask]) / cont(s.spec.x()[mask])
                                xx = s.spec.x()[mask]
                                #wtmp = np.trapz(func_ew, x=s.spec.x()[mask])
                                wtmp = np.cumsum(func_ew[:-1]*np.diff(s.spec.x()[mask]))
                                bx[1].step(xx[:-1],wtmp,where='mid')
                                sbinx =  s.spec.x()[mask]
                                func_ew_err = s.spec.err()[mask] / cont(s.spec.x()[mask])
                                errwtmp = np.sqrt(np.sum(((func_ew_err)[:-1:2]*np.diff(s.spec.x()[mask])[::2])**2))
                                #print('wtmp:', wtmp[-1] / (1+self.parent.z_abs),'errw:=', errwtmp / (1+self.parent.z_abs))
                                if 1:
                                    ew = []
                                    v90 = []
                                    xx = s.spec.x()[mask]
                                    xbin = np.diff(xx)
                                    cc= np.array(cont(s.spec.x()[mask]))
                                    yy0 = s.spec.y()[mask]
                                    yerr = s.spec.err()[mask]
                                    #const =  1/self.parent.abs.reference.line.l() / (1 + self.parent.z_abs) - 1) * ac.c.to('km/s').value
                                    for m in range(500):
                                        delta = np.random.normal(size=len(xx))
                                        yy =  np.abs(yy0 + yerr*delta)
                                        func_ew = (cc - yy) / cc
                                        wtmp = np.cumsum(func_ew[:-1] * xbin)
                                        bx[0].step(xx, (cc-yy)/yerr, where='mid', alpha=0.2, color='grey')
                                        bx[1].step(xx[:-1],wtmp, where='mid', alpha=0.2, color='grey')
                                        ew.append(wtmp[-1] / (1+self.parent.z_abs))
                                        if len(xx)>0:
                                            dv = np.cumsum(np.log(cc / yy))
                                            dv90 = interp1d(dv / dv[-1], (xx/ self.parent.abs.reference.line.l() / (1 + self.parent.z_abs) - 1) * ac.c.to('km/s').value , fill_value='extrapolate')
                                            dv90_x = interp1d(dv / dv[-1], xx,  fill_value='extrapolate')
                                            v90.append(dv90(0.95) - dv90(0.05))
                                            bx[3].step(xx, dv / dv[-1], where='mid', alpha=0.2, color='grey')
                                            bx[3].axvline(dv90_x(0.95), alpha=0.2, color='grey')
                                            bx[3].axvline(dv90_x(0.05), alpha=0.2, color='grey')
                                            #print(dv90(0.05), dv90(0.95),dv90(0.95) - dv90(0.05))
                                    text1 =  'boostrap:'
                                    text1 += 'w_r = {0:0.1f}+/-{1:0.1f}**'.format(np.mean(ew)*1e3,np.std(ew)*1e3)
                                    text1 += 'dv90 = {0:0.1f}+/-{1:0.1f}**'.format(np.mean(v90),np.std(v90))
                                    text1+= ', d90_c = {:0.1f}'.format(np.sqrt((np.mean(v90))**2 - (1.4 * ac.c.to('km/s').value / self.parent.s[self.parent.s.ind].resolution)**2))
                                    print('ew:bootstrap:', np.mean(ew),np.std(ew))
                                    print('v90:bootstrap:', np.mean(v90),np.std(v90))
                                    print(text)
                                if len(s.spec.x()[mask]) > 0:
                                    dv = np.cumsum(np.log(cont(s.spec.x()[mask])/s.spec.y()[mask]))
                                    dv90 = interp1d(dv / dv[-1], (s.spec.x()[mask] / self.parent.abs.reference.line.l() / (1 + self.parent.z_abs) - 1) * ac.c.to('km/s').value,  fill_value='extrapolate')
                                    dv90_x = interp1d(dv / dv[-1], s.spec.x()[mask], fill_value='extrapolate')
                                    print('d90_c = ',np.sqrt((dv90(0.95) - dv90(0.05))**2 - (1.4 * ac.c.to('km/s').value / self.parent.s[self.parent.s.ind].resolution)**2))
                                    text1+=' v_aver = {0:0.1f}'.format(dv90(0.05) + (dv90(0.95)-dv90(0.05))/2)
                                    print(' v_aver = {0:0.1f}'.format(dv90(0.05) + (dv90(0.95)-dv90(0.05))/2))
                                    bx[2].step(s.spec.x()[mask],np.log(cont(s.spec.x()[mask])/s.spec.y()[mask]))
                                    bx[3].step(s.spec.x()[mask] ,dv / dv[-1], where='mid')
                                    bx[3].axvline(dv90_x(0.95))
                                    bx[3].axvline(dv90_x(0.05))
                                    bx[3].axvline(dv90_x(0.05) + (dv90_x(0.95) - dv90_x(0.05)) / 2,color='red',ls='--')
                                    print('err_v90', 1.4 * ac.c.to('km/s').value / self.parent.s[self.parent.s.ind].resolution)
                                    #left,right = bx[0].getxlim()
                                    bx[3].set_xlim(bx[0].get_xlim())
                                    bx[0].set_title(text1,fontsize=10)
                                else:
                                    print('d90_c = NONE')
                                plt.show()
                                self.parent.console.set(text1)
                        else:
...
class fitMCMCWidget(QWidget):
    ...
    def MCMC(self, init=True):
        ...
        self.start_button.setChecked(False)
        with open('output/pars.pkl', 'wb') as f:
            pickle.dump(pars, f)
        ...
        if t == 'fit':
            ...
            for i, p in enumerate(pars):
                ...
                if p in names:
                    ...
                    ax[vert, hor].text(.95, .9, self.parent.fit.getPar(p).fitres(latex=True, showname=False),
            ...
            if len(ratios) > 0:
                ...
                for i in range(len(self.parent.fit.sys)):
                    for ratio in ratios:
                        ...
                        self.results.setText(self.results.toPlainText() + str(
                            '{0:s}_{1:s}_{2:d}'.format(ratio[0], ratio[1], i)) + ': ' + res.latex(f=f) + '\n')
                        ...
                        ax[vert, hor].yaxis.set_ticklabels([])
                        ax[vert, hor].yaxis.set_ticks([])
                        ax[vert, hor].text(.1, .9, str('{0:s}/{1:s} {2:d}'.format(ratio[0], ratio[1], i)), ha='left',
                                           va='top', transform=ax[vert, hor].transAxes)
        else:
            values = []
            ...
            if t == 'all':
                ...
                for i, p in enumerate(self.parent.fit.list()):
                    ...
                    if np.std(values[:, i]) > 0:
                        ...
                        ax[vert, hor].yaxis.set_ticklabels([])
                        ax[vert, hor].yaxis.set_ticks([])
                        ax[vert, hor].text(.1, .9, str(p).replace('_', ' '), ha='left', va='top',
                                           transform=ax[vert, hor].transAxes)
                        # ax[vert, hor].set_title(str(p).replace('_', ' '))

            elif t == 'cols':
                # mask = np.array([p.show for p in self.parent.fit.list_fit()])
                names = [str(p).split('_')[-1] for p in self.parent.fit.list_fit() if p.show]
                print('names', names)
                sp = set()
                for i, sys in enumerate(self.parent.fit.sys):
                    for s in sys.sp.keys():
                        if s not in sp and s in names:
                            sp.add(s)
                    for el in ['H2', 'HD', 'CO', 'CI', 'CII', 'FeII']:
                        if any([el + 'j' in s for s in sys.sp.keys()]) and any([el in p for p in names]):
                ...
                list_fit = [str(p) for p in self.parent.fit.list_fit() if p.show]
                print('sp', sp)
                print('list_fit', list_fit)
                ...
                if n_vert <= 1:
                    n_vert = 2
                print('self.parent.fit.list()', self.parent.fit.list())
                fig, ax = plt.subplots(nrows=n_vert, ncols=n_hor, figsize=(6 * n_vert, 4 * n_hor))
                i = 0
                print('sp:after fig', sp)
                print('print(self.parent.fit.list())',self.parent.fit.list())
                for k, v in sp.items():
                    print(k, v)
                    if 'total' in k:
                        #inds = np.where([str(s) in list_fit and k.split('_')[2] == str(s).split('_')[2] for s in
                        #                 self.parent.fit.list()])[0]
                        #if k.split('_')[-1] == 'total':

                        #inds = np.where([k.split('_')[2] in str(s) and str(s)[0] == 'N' and str(s) in list_fit for s in self.parent.fit.list()])[0]
                        inds = np.where([k.split('_')[2] in str(s) and k.split('_')[2]+'I' not in str(s) and str(s)[0] == 'N' and str(s) in list_fit for s in self.parent.fit.list()])[0]


                        #    #inds = np.where([k.split('_')[2] in str(s) and str(s)[0] == 'N' for s in list_fit])[0]
                        #else:
                        #    inds = np.where([str(s)[0] == 'N' and k.split('_')[2] == str(s).split('_')[2] for s in self.parent.fit.list()])[0]
                        #    #inds = np.where([str(s)[0] == 'N' and k.split('_')[2] == str(s).split('_')[2] for s in list_fit])[0]
                    else:
                        inds = np.where([k[2:] in str(s) and str(s)[0] == 'N' for s in list_fit])[0]
                    if 1:
                        print('inds', inds)
                        if len(inds) > 1:
                            ...
                            v.set(d.point)
                            f = int(np.round(np.abs(np.log10(np.min([res.plus, res.minus])))) + 1)
                            self.results.setText(self.results.toPlainText() + k + ': ' + v.fitres(latex=True, dec=f,
                                                                                                  showname=False) + '\n')
                            ...
                            ax[vert, hor].text(.1, .9, k.replace('_', ' '), ha='left', va='top',
                                               transform=ax[vert, hor].transAxes)
                            ax[vert, hor].text(.95, .9, v.fitres(latex=True, showname=False),
                                               ha='right', va='top', transform=ax[vert, hor].transAxes)
...
class ExportDataWidget(QWidget):
    ...
    def export(self):
        def savefits(filename, wave, flux, err, err_low):
            from astropy.io import fits
            hdr = fits.Header()
            hdr['INSTRUME'] = 'SPECTRO'
            hdr['AUTHOR'] = 'V.KLIMENKO'
            hdr['COMMENT'] = "This file was created by Spectro code."
            ...
            col4 = fits.Column(name='ERROR_LOWER', format='E', array=err_low)
        ...
        if self.cheb_applied.isChecked() and self.parent.fit.cont_fit:
            ...
            cheb = interp1d(s.spec.raw.x[s.cont_mask], np.ones_like(s.spec.raw.x[s.cont_mask]), fill_value=1, bounds_error=False)
        if 1:
            self.filename = '/home/slava/science/codes/python/spectro/sviewer/fits/fit.dat'
        print('def export:', self.filename)

        if self.spectrum.isChecked():
            np.savetxt(self.filename, np.c_[s.spec.x()[mask] / unit, s.spec.y()[mask] / cheb(s.spec.x()[mask]), s.spec.err()[mask],
                                            s.spec.errm()[mask],s.spec.y()[mask] / cheb(s.spec.x()[mask])/s.spec.err()[mask]], **kwargs)
            savefits(filename='/home/slava/science/codes/python/spectro/sviewer/fits/spectrum',wave=s.spec.x()[mask] / unit,flux =s.spec.y()[mask], err= s.spec.err()[mask], err_low=s.spec.errm()[mask])
        ...
        #    np.savetxt('_fit_regions.'.join(self.filename.rsplit('.', 1)), np.c_[s.spec.x()[np.logical_and(mask, fit_mask)] / unit, (s.spec.y() / cheb(s.spec.x()))[np.logical_and(mask, fit_mask)]], **kwargs)
            if 0:
                for k, c in enumerate(s.fit_comp):
                    fit_comp_name = '_fit_comp' + str(k) + '.'
                    fit_comp_name = fit_comp_name.join(self.filename.rsplit('.', 1))
                    comp_mask = np.isfinite(c.x())
                    print('fit_comp_name', fit_comp_name, np.sum(comp_mask))
                    np.savetxt(fit_comp_name, np.c_[c.x()[comp_mask] / unit, c.y()[comp_mask]], **kwargs)
    ...
    def save(self):
        if 0:
            self.filename = '/home/slava/science/codes/python/spectro/sviewer/fits/tmp.spv'
            print('edit filename = ', self.filename)
...
class combineWidget(QWidget):
    ...
    def combine(self):
        ...
        e_comb.fill(np.nan)
        e_lower_comb = np.empty([len(slist), len(x)], dtype=np.float)
        e_lower_comb.fill(np.nan)
        ...
        for i, s in enumerate(slist):
            if 1:
                ...
                #comb[i][mask], e_comb[i][mask],e_lower_comb[i][mask] = spectres.spectres(s.spec.x()[mask_s], s.spec.y()[mask_s], x[mask],
                #                                                                         spec_errs=s.spec.err()[mask_s], spec_errs_lower=s.spec.errm()[mask_s])
                comb[i][mask], e_comb[i][mask],e_lower_comb[i][mask] = spectres.spectres(s.spec.x()[mask], s.spec.y()[mask], x[mask],
                                                                                         spec_errs=s.spec.err()[mask], spec_errs_lower=s.spec.errm()[mask])
            else:
                mask_s = (s.spec.err() == 0)
                comb[i], e_comb[i], e_lower_comb[i] = s.spec.y(),s.spec.err(),s.spec.errm()
                comb[i][mask_s], e_comb[i][mask_s], e_lower_comb[i][mask_s] = 0,np.inf,np.inf
        print(comb, e_comb,e_lower_comb)
        ...
        if typ == 'Median':
            ...
            errm = np.power(np.nansum(np.power(e_lower_comb, -2), axis=0), -0.5)
...
class subtractWidget(QWidget):
    def __init__(self, parent):
        ...
        vbox = QVBoxLayout()
        hbox = QHBoxLayout()
        hbox.addWidget(QLabel('subtract type: '))
        self.selectcombtype = QComboBox(self)
        self.selectcombtype.addItems(['Normal'])
        self.selectcombtype.setFixedSize(150, 30)
        self.selectcombtype.setCurrentIndex(0)
        hbox.addWidget(self.selectcombtype)
        hbox.addStretch(1)
        ...
        self.combineButton = QPushButton("Subtact")
        self.combineButton.setFixedSize(90, 30)
        self.combineButton.clicked[bool].connect(self.subtract)
        self.cancelButton = QPushButton("Cancel")
        self.cancelButton.setFixedSize(90, 30)
        self.cancelButton.clicked[bool].connect(self.close)
        hbox = QHBoxLayout()
    ...
    def subtract(self):
        ...
        for i, s in enumerate(slist):
            ...
            if i == 1:
                #mask_s = (s.spec.err() == 0)
                mask_e =(s.spec.err() != 0)*(s.spec.x()>=x[0])*(s.spec.x()<=x[-1])
                spec = s.spec.y()[:]
                from scipy import signal
                kernel = signal.gaussian(100, std=20)
                spec = signal.convolve(spec, kernel, mode='same') / sum(kernel)
                fe = interp1d(s.spec.x()[mask_e],spec[mask_e], fill_value='extrapolate')
                comb[i] = -fe(x)
                fep = interp1d(s.spec.x()[mask_e], s.spec.err()[mask_e], fill_value='extrapolate')
                e_comb[i] = fep(x)/3
                fem = interp1d(s.spec.x()[mask_e], s.spec.errm()[mask_e], fill_value='extrapolate')
                e_lower_comb[i] = fem(x)/3
                #conv.convolve(spec, kernel, boundary="extend", normalize_kernel=True, )
                #comb[i], e_comb[i], e_lower_comb[i] = -spec, s.spec.err(), s.spec.errm()
                #comb[i][mask_s], e_comb[i][mask_s], e_lower_comb[i][mask_s] = 0,np.inf,np.inf
...
class rebinWidget(QWidget):
    ...
    def rebin_weight_mean(self, y, errp,errm,factor):
        wp = np.array(np.power(errp,-2))
        wm = np.array(np.power(errm,-2))
        w = np.array(np.power(errm*0.5+errp*0.5,-2))
        a = np.array(y)
        n = a.shape[0] // factor
        a*=w
        a = a[:n*factor].reshape(n, factor)
        wp = wp[:n * factor].reshape(n, factor)
        wm = wm[:n * factor].reshape(n, factor)
        w = w[:n * factor].reshape(n, factor)
        a = a.sum(1)
        w = w.sum(1)
        wp = wp.sum(1)
        wm = wm.sum(1)
        return a/w,np.power(wp,-0.5),np.power(wm,-0.5)

    def rebin_aunc(self, y,errp,errm, factor):
        n = y.shape[0] // factor
        ynew = y[:n*factor].reshape(n, factor)
        errpnew = errp[:n*factor].reshape(n, factor)
        errmnew = errm[:n * factor].reshape(n, factor)
        yout = np.zeros(n)
        errpout = np.zeros(n)
        errmout = np.zeros(n)
        for i in range(n):
            print(i)
            f,p,m = ynew[i,:],errpnew[i,:],errmnew[i,:]
            s = a(0,2,2)
    ...
    def rebin(self):
        if self.fixednumber_item.isExpanded():
            ...
            if 1:
                n = int(self.binnum.text())
                x = self.rebin_arr(self.parent.s[self.exp_ind].spec.x(), n)
                y, errp, errm = self.rebin_weight_mean(self.parent.s[self.exp_ind].spec.y(),
                        self.parent.s[self.exp_ind].spec.err(),self.parent.s[self.exp_ind].spec.errm(),n)
            if 0:
                n = int(self.binnum.text())
                x = self.rebin_arr(self.parent.s[self.exp_ind].spec.x(), n)
                y,errp,errm = self.rebin_aunc(self.parent.s[self.exp_ind].spec.y(),self.parent.s[self.exp_ind].spec.err(),
                            self.parent.s[self.exp_ind].spec.errm(), n)
            self.parent.s.append(Spectrum(self.parent, name='rebinned '+str(self.exp_ind+1), data=[x, y, errp,errm,errm]))
...
class sviewer(QMainWindow):
    ...
    def initStatus(self):
        ...
        self.abs_SF_status = 0
        self.abs_LLS_status = 1
        self.abs_IR_Metal_status = 0
        self.abs_IR_Molec_status = 0
        self.abs_IR_atomic_status = 0
        self.normview = False
        self.aodview = False
    ...
    def initMenu(self):
        ...
        self.linesLLS = QAction('&LLS lines', self, checkable=True)
        self.linesLLS.setStatusTip('Add LLS lines')
        self.linesLLS.triggered.connect(partial(self.absLines, 'abs_LLS_status'))
        self.linesLLS.setChecked(self.abs_LLS_status)

        self.linesIRMetal = QAction('&IR Metal lines', self, checkable=True)
        self.linesIRMetal.setStatusTip('Add IR Metal lines')
        self.linesIRMetal.triggered.connect(partial(self.absLines, 'abs_IR_Metal_status'))
        self.linesIRMetal.setChecked(self.abs_IR_Metal_status)

        self.linesIR_atomic = QAction('&IR atomic lines', self, checkable=True)
        self.linesIR_atomic.setStatusTip('Add IR atomic lines')
        self.linesIR_atomic.triggered.connect(partial(self.absLines, 'abs_IR_atomic_status'))
        self.linesIR_atomic.setChecked(self.abs_IR_atomic_status)

        self.linesIR_Molec = QAction('&IR Molec lines', self, checkable=True)
        self.linesIR_Molec.setStatusTip('Add IR Molec lines')
        self.linesIR_Molec.triggered.connect(partial(self.absLines, 'abs_IR_Molec_status'))
        self.linesIR_Molec.setChecked(self.abs_IR_Molec_status) #IR_Molec_list
        ...
        linesMenu.addAction(self.linesLLS)
        linesMenu.addAction(self.linesMolec)
        linesMenu.addAction(self.linesSF)
        linesMenu.addAction(self.linesIRMetal)
        linesMenu.addAction(self.linesIR_atomic)
        linesMenu.addAction(self.linesIR_Molec)
        ...
        subtract.setStatusTip('Subtract exposures')
        ...
        combineMenu.addAction(subtract)
    ...
    def absLines(self, status='', sig=True, value=None, verbose=False):
        ...
        if value != 0:
            ...
            if status == 'abs_LLS_status':
                lines, color, va = self.atomic.LLS_COS_list(), (255, 111, 63), 'down'
            if status == 'abs_IR_Metal_status':
                lines, color, va = self.atomic.IR_Metal_list(), (255, 111, 63), 'down'
            if status == 'abs_IR_atomic_status':
                lines, color, va = self.atomic.IR_atomic_list(), (105, 213, 105), 'down'
            if status == 'abs_IR_Molec_status':
                lines, color, va = self.atomic.IR_Molec_list(), (229, 43, 80), 'down'
            if verbose:
    ...
    def check_hdulist(self, hdulist):

        if 'OBJECT' in hdulist[0].header:
            objname = hdulist[0].header['OBJECT']
            print('Object name:', objname)
            if objname == 'Hip085967':
                # HD158928
                l = 345.12
                b = -6.56
            elif objname ==  'HD_156575':
                l_coord = 342.94
                b_coord = -5.15
            elif objname == 'HD_156385' or 'HD-156385':
                l_coord = 343.16
                b_coord = -4.76
            elif objname ==  'V636_Sco':
                l_coord = 343.51
                b_coord = -5.22
            elif objname ==  'HD 157832':
                l_coord = 342.8
                b_coord = -6.74
            elif objname ==  'HD_154873':
                l_coord = 341.34
                b_coord = -4.10
            elif objname ==  'V830Ara':
                l_coord = 344.57
                b_coord = -6.86
            elif objname ==  'HD156398':
                l_coord = 344.32
                b_coord = -3.94
            elif objname == 'V507 SCO':
                l_coord = 344.77
                b_coord = -4.70
            elif objname == '17330310-4259316':
                l_coord = 346.72
                b_coord = -5.31
            elif objname == 'HD-155985':
                l_coord = 343.62
                b_coord = -3.42
            elif objname == 'HD156662':
                l_coord = 343.04
                b_coord = -5.18
            elif objname == 'HD158042':
                l_coord = 345.49
                b_coord = -5.23
            elif objname == 'HD157Sco':
                l_coord = 344.77
                b_coord = -4.70
            elif objname == 'HD156398':
                l_coord = 344.32
                b_coord = -3.94
            elif objname == 'HD155756':
                l_coord = 342.56
                b_coord = -4.38
        elif 'FILENAME' in hdulist[0].header:
            if hdulist[0].header['FILENAME'] == 'mbgphH201906190017_u1wm.bdf':
                # HD 157573
                l_coord = 344.73
                b_coord = -5.13
            elif hdulist[0].header['FILENAME'] == 'mbgphH201906190018_u1wm.bdf':
                # HD158078
                l_coord = 343.24
                b_coord = -6.77
            elif hdulist[0].header['FILENAME'] == 'mbgphH201907100038_u1wm.bdf':
                # HD158928
                l_coord = 345.12
                b_coord = -6.56
        elif  hdulist[0].header['ORIGIN'] == 'ESO-MIDAS':
            if 'FILENAME' in hdulist[0].header:
                if hdulist[0].header['FILENAME'] == 'mbgphR201906190017_u1wm.fits':
                    # HD 157573
                    l_coord = 344.73
                    b_coord = -5.13
                elif hdulist[0].header['FILENAME'] == 'mbgphR201906190018_u1wm.tbl':
                    # HD158078
                    l_coord = 343.24
                    b_coord = -6.77
            elif 'FILENAME' in hdulist[1].header:
                if hdulist[1].header['FILENAME'] == 'mbgphR201907100038_u1wm.tbl':
                    # HD158928
                    l_coord = 345.12
                    b_coord = -6.56
                elif hdulist[1].header['FILENAME'] == 'Spec_202008010020.tbl':
                    # HD157115
                    l_coord = 343.78
                    b_coord = -5.24
                elif hdulist[1].header['FILENAME'] == 'Spec_202008020028.tbl':
                    # HD157832
                    l_coord = 342.82
                    b_coord = -6.74
        return l_coord,b_coord

    def check_vel_hel(self, hdulist):

        vel_helio = 0.0
        if 'HIERARCH ESO QC VRAD HELICOR' in hdulist[0].header:
            vel_helio = hdulist[0].header['HIERARCH ESO QC VRAD HELICOR']
        elif 'FILENAME' in hdulist[0].header:
            if hdulist[0].header['FILENAME'] == 'mbgphH201906190017_u1wm.bdf':
                # HD 157573
                vel_helio = -2.36849
                ...
                # HD158928
                v = -10.9349
                vel_helio = float(-10.9349)
        elif 'ORIGIN' in hdulist[0].header and hdulist[0].header['ORIGIN'] == 'ESO-MIDAS':
            if 'FILENAME' in hdulist[0].header and hdulist[0].header['FILENAME'] == 'mbgphR201906190017_u1wm.fits':
                # HD 157573
                vel_helio = -2.36849
            elif hdulist[1].header['FILENAME'] == 'mbgphR201906190018_u1wm.tbl':
                # HD158078
                vel_helio = -2.00231
            elif hdulist[1].header['FILENAME'] == 'mbgphR201907100038_u1wm.tbl':
                # HD158928
                vel_helio = -10.9349
            elif hdulist[1].header['FILENAME'] == 'Spec_202008010020.tbl':
                # HD157115
                ...
                # HD157832
                vel_helio = -19.93
        elif 'INSTRUME' in hdulist[0].header and hdulist[0].header['INSTRUME'] == 'FEROS':
            vel_helio = hdulist[0].header['HIERARCH ESO DRS BARYCORR']
        if 'SPECSYS' in hdulist[0].header and hdulist[0].header['SPECSYS'] == 'BARYCENT':
            vel_helio  = 0.0
        print('Helio_velocity is', vel_helio)
        if vel_helio is None:
            vel_helio = 0.0
            print('Helio_velocity corr is not found', vel_helio)

        return vel_helio


    def importSpectrum(self, filelist, spec=None, mask=None, header=0, dir_path='', scale_factor=1, append=False, corr=True, lsr_shift=True):
                ...
                if 'IGMspec' in filename:
                    ...
                    if 'INSTRUME' in hdulist[0].header:
                        try:
                            ...
                            if any([instr in hdulist[0].header['INSTRUME'] for instr in ['UVES', 'VIMOS','HARPS']]):
                                prihdr = hdulist[1].data
                                l = prihdr[0][0][:]
                                coef = 1e17 if 'VIMOS' in hdulist[0].header['INSTRUME'] else 1
                                if hdulist[0].header['INSTRUME'] == 'HARPS':
                                    snr = float(hdulist[0].header['SNR'])
                                    err = prihdr[0][1][:] / snr
                                    s.set_data([l, prihdr[0][1][:]*coef, err*coef])
                                else:
                            ...
                            if 'FUSE' in hdulist[0].header['TELESCOP']:
                                prihdr = hdulist[1].data
                                l,f,err = prihdr['WAVE'][0][:],prihdr['FLUX'][0][:],prihdr['SIGMA'][0][:]
                                print(l,f,err)
                                s.set_data([l, f, err])
                                #print(s)
                            ...
                            if 'COS' in hdulist[0].header['INSTRUME']:
                                prihdr = hdulist[1].data
                                x1d_fuva = prihdr['segment'] == 'FUVA'
                                x1d_fuvb = prihdr['segment'] == 'FUVB'
                                ...
                                flux_norm_val = 1e17
                                coswavel = np.append(prihdr[x1d_fuvb]['WAVELENGTH'],prihdr[x1d_fuva]['WAVELENGTH'])
                                cosflux  = np.append(prihdr[x1d_fuvb]['FLUX'],prihdr[x1d_fuva]['FLUX'])
                                coserrp = np.append(prihdr[x1d_fuvb]['ERROR'],prihdr[x1d_fuva]['ERROR'])
                                coserrm = np.append(prihdr[x1d_fuvb]['ERROR_LOWER'],prihdr[x1d_fuva]['ERROR_LOWER'])
                                cosgcounts = np.append(prihdr[x1d_fuvb]['GCOUNTS'],prihdr[x1d_fuva]['GCOUNTS'])
                                s.set_data([coswavel,flux_norm_val * cosflux,flux_norm_val * coserrp,flux_norm_val * coserrm,cosgcounts])
                                if 0:
                                    s.set_data([prihdr[x1d_fuva]['WAVELENGTH'],
                                            flux_norm_val * prihdr[x1d_fuva]['FLUX'],
                                            flux_norm_val * prihdr[x1d_fuva]['ERROR'],
                                            flux_norm_val * prihdr[x1d_fuva]['ERROR_LOWER'],
                                            prihdr[x1d_fuva]['GCOUNTS']
                                            ])
                                    s.wavelmin = np.min(s.spec.raw.x)
                                    s.wavelmax = np.max(s.spec.raw.x)
                                    s.filename +='_a'
                                    ...
                                    s= Spectrum(self, name=filename+'_b')
                                    s.set_data([prihdr[x1d_fuvb]['WAVELENGTH'],
                                                flux_norm_val * prihdr[x1d_fuvb]['FLUX'],
                                                flux_norm_val * prihdr[x1d_fuvb]['ERROR'],
                                                flux_norm_val * prihdr[x1d_fuvb]['ERROR_LOWER'],
                                                prihdr[x1d_fuvb]['GCOUNTS']
                                                ])
                                s.wavelmin = np.min(s.spec.raw.x)
                                s.wavelmax = np.max(s.spec.raw.x)
                                s.insrument = 'COS'
                                ...
                                corr = False
                                if 0:
                                    with open('./data/hst/hst-kernel.pkl', 'rb') as f:
                                        new_lsf, new_w, step = pickle.load(f)
                                    s.instr_funct['lsf'] = new_lsf
                                    s.instr_funct['w'] = new_w
                                    s.instr_funct['step'] = step

                            if 'SPECTRO' in hdulist[0].header['INSTRUME']:
                                prihdr = hdulist[1].data
                                coswavel = np.array(prihdr['WAVELENGTH'])
                                cosflux = np.array(prihdr['FLUX'])
                                coserrp = np.array(prihdr['ERROR'])
                                if 'ERROR_LOWER' in prihdr.header:
                                    flux_norm_val = 1
                                    coserrm = np.array(prihdr['ERROR_LOWER'])
                                    s.set_data([coswavel, flux_norm_val * cosflux, flux_norm_val * coserrp,
                                                flux_norm_val * coserrm, cosflux / coserrp])
                                else:
                                    flux_norm_val = 1
                                    s.set_data([coswavel, flux_norm_val * cosflux, flux_norm_val * coserrp])
                            ...
                            if hdulist[0].header['INSTRUME'] == 'FEROS':
                                prihdr = hdulist[1].data
                                l = prihdr[0][0][:]
                                coef = 1
                                snr = float(hdulist[0].header['SNR']) / 5
                                s.set_data([l, prihdr[0][1][:] * coef, prihdr[0][1][:] * coef / snr])
                                if 'SPEC_RES' in hdulist[0].header:
                                    s.resolution = hdulist[0].header['SPEC_RES']
                                if 'DATE-OBS' in hdulist[0].header:
                                    s.date = hdulist[0].header['DATE-OBS']
                                print('SPEC_RES', s.resolution, s.date)

                            if 'FUV' in hdulist[0].header['INSTRUME'] and 0:
                                prihdr = hdulist[1].data
                                s.set_data([prihdr['WAVE'], prihdr['FLUX'] * 1e17, prihdr['ERROR'] * 1e17])

                            if hdulist[0].header['INSTRUME'] == 'HRS':
                                image = hdulist[0].data
                                header = hdulist[0].header
                                # read wavelenght and spatial scale
                                ...
                                if len(image.shape) > 1:
                                    ...
                                    i = 0
                                    #
                                range_wave = crval1 + cdelt1 * (np.array([i, image.shape[i]]) - crpix1)
                                wave = np.zeros_like(image[1, :])
                                flux = np.zeros_like(wave)
                                errflux = np.zeros_like(wave)
                                for k, el in enumerate(image[1, :]):
                                    wave[k] = crval1 + cdelt1 * (k)
                                    flux[k] = image[0, k]
                                    errflux[k] = image[1, k]
                                s.set_data([wave, flux, errflux])


                            if 'MIRI' in hdulist[0].header['INSTRUME']:
                                prihdr = hdulist[1].data
                                coswavel = np.array(prihdr['WAVELENGTH'])
                                cosflux = np.array(prihdr['FLUX'])
                                coserrp = np.array(prihdr['ERROR'])
                            ...
                            if 'IRS' in hdulist[0].header['INSTRUME']:
                                prihdr = hdulist[1].data
                                coswavel = np.array(prihdr['WAVELENGTH'])
                                cosflux = np.array(prihdr['FLUX'])
                                coserrp = np.array(prihdr['ERROR'])
                            ...
                            try:
                                if corr:
                                    ...
                                    s.helio_vel = self.check_vel_hel(hdulist=hdulist)
                                    if lsr_shift:
                                        l,b = self.check_hdulist(hdulist=hdulist)
                                        lsr_vel = s.calc_vel_lsr(l=l,b=b)
                                        s.apply_shift(s.helio_vel+lsr_vel)
                                        print(s.helio_vel,lsr_vel)
                                    else:
                        ...
                        if hdulist[0].header['ORIGIN'] == 'ESO-MIDAS':
                            ...
                            s.set_data([prihdr['WAVE'], prihdr['FLUX'], prihdr['ERROR']])
                            print('SNR', np.mean(prihdr['FLUX'] / prihdr['ERROR']))
                        if corr:
                            print('apply corrections:')
                            # s.helio_vel = hdulist[0].header['HIERARCH ESO QC VRAD HELICOR']
                            s.helio_vel = self.check_vel_hel(hdulist=hdulist)
    ...
    def fitGauss(self, kind='integrate'):
        ...
        for s in self.s:
            ...
            if n > 0:
                ...
                x, y, err = np.array(s.spec.x()[s.mask.x()], dtype=np.float), np.array(s.spec.y()[s.mask.x()], dtype=np.float), \
                            np.array(s.spec.err()[s.mask.x()], dtype=np.float)
                if self.normview:
                ...
                if 1:
                    ...
                    c = ChainConsumer()
                    names, truth = ['Area', 'Centroid', 'Amplitude'], [params['amp'].value, params['cen'].value,
                                                                       params['amp'].value / (2 * np.pi)**0.5 / params['disp'].value]
                ...
                print('Gaussian:',amp, mean, sigma)
                print('Integrated flux:', gauss_integ_flux(amp=amp),gauss_integ_flux(amp=amp+result.params['amp'].stderr))
    ...
    def subtract(self):
        """
        combine exposuressubtract
        """

        self.subtractWidget = subtractWidget(self)
        self.subtractWidget.show()
